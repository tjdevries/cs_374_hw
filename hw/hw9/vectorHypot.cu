...
int main(int argc, char** argv)
{
    ...
    printf("[Vector calculation of %lu elements]\n", numElements);
    ...
    double cpyToDevice1 = omp_get_wtime();
    ...
    double cpyToDevice2 = omp_get_wtime();
    double cpyToDevice = cpyToDevice2 - cpyToDevice1;
    printf("Time spent coping from host to device: %f\n", cpyToDevice);


    // 4. Launch the Vector Add CUDA Kernel
    int threadsPerBlock = 256;
    int blocksPerGrid =(numElements + threadsPerBlock - 1) / threadsPerBlock;
    //printf("CUDA kernel launch with %d blocks of %d threads\n", blocksPerGrid, threadsPerBlock);
    double ComputeTime1 = omp_get_wtime();
    vectorHypot<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, numElements);
    double ComputeTime2 = omp_get_wtime();
    double ComputeTime = ComputeTime2 - ComputeTime1;
    printf("Time spent computing the vector squares: %f\n", ComputeTime);
    ...
    double cpyTimeToHost1 = omp_get_wtime();
    err = cudaMemcpy(h_C, d_C, size, cudaMemcpyDeviceToHost);
    double cpyTimeToHost2 = omp_get_wtime();
    double cpyTimeToHost = cpyTimeToHost2 - cpyTimeToHost1;
    printf("Time spent copying the C array from the device to the host: %f\n", cpyTimeToHost);
    checkErr(err, "Failed to copy vector C from device to host");

    printf("Total time: %f\n", cpyTimeToHost + ComputeTime + cpyToDevice);
    ...
    double seq1 = omp_get_wtime();
    for (int i = 0; i < numElements; ++i)
    {
       h_C[i] = sqrt(h_A[i] * h_A[i] + h_B[i] * h_B[i]);
    }
    double seq2 = omp_get_wtime();
    // verify again
    for (int i = 0; i < numElements; ++i)
    {
        if (fabs(sqrt(h_A[i] * h_A[i] + h_B[i] * h_B[i]) - h_C[i]) > 1e-5)
